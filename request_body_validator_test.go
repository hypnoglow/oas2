...
func TestBodyValidator(t *testing.T) {
	...
	router, err := NewRouter(loadDoc().Spec(), handlers, MiddlewareOpt(NewBodyValidator(validationErrorsHandler)))
	if err != nil {
		t.Fatalf("Unexpected error: %s", err)
	}

	t.Run("positive", func(t *testing.T) {
		resp := helperPost(t, router, "/pet", bytes.NewBufferString(`{"name":"johndoe", "age":7}`))
	...
	t.Run(`required field "name" is missing`, func(t *testing.T) {
		resp := helperPost(t, router, "/pet", bytes.NewBufferString(`{"age":7}`))
	...
	t.Run(`value for field "age" is incorrect`, func(t *testing.T) {
		resp := helperPost(t, router, "/pet", bytes.NewBufferString(`{"name":"johndoe","age":"abc"}`))
	...
	t.Run("no body", func(t *testing.T) {
		resp := helperPost(t, router, "/pet", nil)
		expectedPayload := ""
		if !bytes.Equal([]byte(expectedPayload), resp) {
			t.Fatalf("Expected response body to be\n%s\nbut got\n%s", expectedPayload, string(resp))
		}
	...
	t.Run("invalid json body", func(t *testing.T) {
		resp := helperPost(t, router, "/pet", bytes.NewBufferString(`{"name":"johndoe`))
	...
	handler := NewBodyValidator(validationErrorsHandler).Apply(resourceHandler)
